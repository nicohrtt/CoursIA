...
public class NotebookUpdater
{
	...
	public async Task<string> UpdateNotebook()
	{
		...
		var plannerPrompt = $"Help create the following interactive .Net notebook, which contains a description of its purpose.\n Use function calling to improve and validate it with incremental edits:\n - Use the UpdateSmallWorkbookCell function to edit targeted cells within the notebook initially, and then UpdateLargeWorkbookCell as they grow in size. \n- Use the RunNotebook on a regular basis after batches of edit to re-execute the notebook and produce outputs, checking if there are any issues, and do further edits to fix all errors found in the json outputs.\n Make sure to edit Markdown cells to explain your subgoals, and to have code cells use outputs for sanity checks of intermediate results.\n- Continue updating the notebook until it runs without  errors and the code cells are filled with code that fully accomplishes the requested task.\n\n{notebookJson}\nRemember, only once the workbook was thoroughly executed and tested should you call the SendFinalAnswer function.";
